...
- **No PIN gate on Settings.** × Blitz gates its Settings screen behind a hardcoded PIN (`1337`, `PinGate.vue`) as a soft non-security speed-bump. That was tried here too and removed per explicit feedback — the gear icon on `Home.vue` goes straight to `Settings.vue`. Don't reintroduce `PinGate.vue`.
